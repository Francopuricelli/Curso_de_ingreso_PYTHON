...
class App(customtkinter.CTk):
    ...
    def btn_convertir_c_f_on_click(self):
        ...
        centigrados_a_numero = float(centigrados)
        centigrados_a_farenheit = (centigrados_a_numero * 9 / 5) - 32
        alert("CONVERSION A FARENHEIT",f"El resultado es: {centigrados_a_farenheit} ")
    def btn_convertir_f_c_on_click(self):
        Fahrenheit = self.txt_temperatura_f.get()
        farenheit_a_numero = float(Fahrenheit)
        Fahrenheit_a_centigrados = (farenheit_a_numero - 32) * 5 / 9
        alert("CONVERSION A CENTIGRADOS",f"El resultado es: {Fahrenheit_a_centigrados}")
